...
def gather_k(gc, worker, started_with_karbonite, home_loc):
    ...
    try:
        ...
        for location in started_with_karbonite:
            if home_loc.distance_squared_to(location) < home_loc.distance_squared_to(first_go):
                first_go = location
        ...
        if gc.can_harvest(worker.id, worker.location.map_location().direction_to(home_loc))==True:
            gc.harvest(worker.id, worker.location.map_location().direction_to(home_loc))
        else:
